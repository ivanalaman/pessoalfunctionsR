plot.larS <-
 function(x, xvar=c("norm","df","arc.length","step"), breaks = TRUE, plottype = c("coefficients", "Cp"), 
          omit.zeros = TRUE, eps = 1e-10, xlab=NULL,ylab=NULL,...)
 {
  object <- x
  ...
  if(plottype == "Cp") {
   ...
   matplot(s1, coef1, xlab = xlab, ..., type = "b", 
           pch = "*", ylab = ylab)
   #title(object$type,line=2.5)
   abline(h = 0, lty = 3)
   axis(4, at = coef1[nrow(coef1),  ], labels = paste(cnums), cex = 0.80000000000000004, adj = 0)
   if(breaks) {
